...
def contentcrawler(link,date,uid):
    try:
        ...
        try:
            ...
            newshead=soup.find('h1',{"class":'heading1'}).text
            
            textsoup = soup.find('div',{"class":'section1'})
            if(textsoup.find('div',{"class":'readalso'})):
                textsoup.find('div',{"class":'readalso'}).decompose()
            newstext = textsoup.text
            
            imagesoup = soup.find('section',{"class":'highlight clearfix'}).find('img')
            imagelink = 'http://timesofindia.indiatimes.com//'+ imagesoup['src']
            ...
            imagedir = os.getcwd()+'/timesofindia_image/'
            imagename = imagedir+ str(uid)+'.jpg'
            f = open(imagename,'wb')
            f.write(image)
            f.close()
            
            outputfile='timesofindia_news.csv'
        ...
        try:
            ...
            for linksoup in linksoups:
                for link in linksoup.find_all('a', href=True):
                    templink=link['href']
                    if (templink.find('http://timesofindia.indiatimes.com//')==-1):
                        templink='http://timesofindia.indiatimes.com/'+templink
                    newslinks+=[templink]
...
def linksgenerator(startdate,enddate,lastuid):
    ...
    for i in range(delta.days + 1):
        ...
        mainlink = 'http://timesofindia.indiatimes.com/'+str(currdate.year)+'/'+str(currdate.month)+'/'+str(currdate.day)+'/archivelist/year-'+str(currdate.year)+',month-'+str(currdate.month)+',starttime-'+str((currdate-basedate).days+2)+'.cms'
        linkscrawler(mainlink,currdate,lastuid)

def main():
    if not os.path.exists(os.getcwd()+'/timesofindia_news.csv'):
        outputfile='timesofindia_news.csv'
        ...
        lastuid = get_last_row('timesofindia_news.csv')
