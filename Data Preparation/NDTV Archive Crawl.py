...
def contentcrawler(link,uid):
    try:
        ...
        try:
            ...
            newshead=soup.find('h1',{"itemprop":'headline'}).text
            
            textsoup = soup.find('div',{"itemprop":'articleBody'})
            newstext = textsoup.text
            
            imagesoup = soup.find('div',{"class":'ins_left_rhs'}).find('img')
            imagelink = imagesoup['src']
            image = urllib2.urlopen(imagelink).read()

            datesoup = soup.find('span',{"itemprop":'dateModified'})['content']
            ...
            imagedir = os.getcwd()+'/ndtv_image/'
            imagename = imagedir+ str(uid)+'.jpg'
            f = open(imagename,'wb')
            f.write(image)
            f.close()
            
            outputfile='ndtv_news.csv'
    ...
    try:
        ...
        try:
            ...
            for link in linksoup.find_all('a', href=True):
                templink=link['href']
                if (templink.find('http://www.ndtv.com/')==0):
                    newslinks+=[templink]
...
def linksgenerator(startdate,enddate,lastuid):
    ...
    for year in range(startdate.year,enddate.year+1):
        for month in range(startmonth[index],endmonth[index]+1):
            mainlink='http://archives.ndtv.com/articles/'+str(year)+'-'+str("%02d" % month)+'.html'
            linkscrawler(mainlink,[year,month],lastuid)
        index+=1

def main():
    
    if not os.path.exists(os.getcwd()+'/ndtv_news.csv'):
        outputfile='ndtv_news.csv'
        ...
        lastuid = get_last_row('ndtv_news.csv')
